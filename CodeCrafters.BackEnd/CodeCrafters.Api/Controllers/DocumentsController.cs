...
public class DocumentsController(IDocumentVaultService documentVaultService) : ControllerBase
{
    ...
    public async Task<ActionResult<UserDocumentDto>> Upload([FromForm] string documentType, [FromForm] IFormFile file, CancellationToken cancellationToken)
    {
        if (file == null || file.Length == 0)
            return BadRequest(new { message = "No file uploaded." });
        var dt = string.IsNullOrWhiteSpace(documentType) || !AllowedDocumentTypes.Contains(documentType, StringComparer.OrdinalIgnoreCase)
            ? "Other"
            : documentType;
